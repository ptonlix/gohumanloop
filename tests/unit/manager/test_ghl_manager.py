...
class TestGoHumanLoopManager(IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.mock_ghl_provider.check_request_status = AsyncMock(return_value=self.result)
        self.mock_ghl_provider.check_conversation_status = AsyncMock(return_value=self.result)
    ...
    async def test_get_ghl_provider(self):
        ...
        # 模拟 get_provider 方法
        with patch.object(manager, 'get_provider', return_value=self.mock_ghl_provider) as mock_get_provider:
    ...
    async def test_cancel_conversation(self):
        ...
        with patch('gohumanloop.core.manager.DefaultHumanLoopManager.cancel_conversation', 
            ...
            self.assertEqual(manager._cancelled_conversations["test-conversation"]["provider_id"], "GoHumanLoop")
            self.assertEqual(manager._cancelled_conversations["test-conversation"]["request_ids"], ["test-request"])
    ...
    async def test_check_request_status(self):
        ...
        self.mock_ghl_provider.check_request_status = AsyncMock(return_value=approved_result)
        ...
        with patch.object(manager, 'async_data_to_platform') as mock_sync:
            ...
            with patch.object(manager, '_trigger_update_callback') as mock_trigger:
                # 添加回调
                manager._callbacks = {("test-conversation", "test-request"): MagicMock()}
                
                # 执行检查请求状态
                result = await manager.check_request_status("test-conversation", "test-request")
                
                # 验证是否调用了提供者的方法
                self.mock_ghl_provider.check_request_status.assert_called_once_with("test-conversation", "test-request")
