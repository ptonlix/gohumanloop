...
class MockBaseProvider(BaseProvider):
    ...
    async def request_humanloop(self, task_id, conversation_id, loop_type, context, metadata=None, timeout=None):
        request_id = self._generate_request_id()
        self._store_request(conversation_id, request_id, task_id, loop_type, context, metadata or {}, timeout)
    ...
    async def continue_humanloop(self, conversation_id, context, metadata=None, timeout=None):
        ...
        self._store_request(conversation_id, request_id, task_id, HumanLoopType.CONVERSATION, context, metadata or {}, timeout)
...
class TestBaseProviderInit(TestCase):
    ...
    def test_str_representation(self):
        """测试字符串表示"""
        provider = MockBaseProvider(name="test_provider")
        self.assertEqual(str(provider), "conversations=0, total_requests=0, active_requests=0)")
        self.assertEqual(repr(provider), "conversations=0, total_requests=0, active_requests=0)")
...
class TestRequestStorage(TestCase):
    ...
    def test_store_request(self):
        ...
        provider._store_request(conversation_id, request_id, task_id, loop_type, context, metadata, timeout)
        ...
        self.assertEqual(provider._conversations[conversation_id]["latest_request_id"], request_id)
...
class TestRequestStatusUpdate(TestCase):
    ...
    def test_update_request_status_error(self):
        ...
        provider._update_request_status_error(conversation_id, request_id, error_message)
...
class TestTimeoutTask(IsolatedAsyncioTestCase):
    ...
    async def test_timeout_task_with_inprogress_status(self):
        ...
        # 将请求状态设置为进行中
        provider._requests[(conversation_id, request_id)]["status"] = HumanLoopStatus.INPROGRESS
...
class TestCancelConversation(IsolatedAsyncioTestCase):
    ...
    async def test_cancel_conversation_with_completed_requests(self):
        ...
        # 存储一个待处理请求和一个已完成请求
        provider._store_request(conversation_id, pending_request_id, task_id, loop_type, context, metadata, timeout)
        provider._store_request(conversation_id, completed_request_id, task_id, loop_type, context, metadata, timeout)
        
        # 将一个请求标记为已完成
        provider._requests[(conversation_id, completed_request_id)]["status"] = HumanLoopStatus.COMPLETED
        ...
        # 验证待处理请求状态是否已更新为取消
        pending_request_info = provider._get_request(conversation_id, pending_request_id)
        self.assertEqual(pending_request_info["status"], HumanLoopStatus.CANCELLED)
        
        # 验证已完成请求状态是否保持不变
        completed_request_info = provider._get_request(conversation_id, completed_request_id)
