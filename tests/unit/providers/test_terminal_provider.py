...
class TestTerminalInteraction(IsolatedAsyncioTestCase):
    ...
    async def test_process_terminal_interaction_approval(self):
        ...
        # 存储请求信息
        provider._store_request(conversation_id, request_id, task_id, 
                               HumanLoopType.APPROVAL, request_info["context"], 
                               request_info["metadata"], None)
        ...
        with patch('asyncio.get_event_loop') as mock_loop:
            ...
            # 调用处理函数
            await provider._handle_approval_interaction(conversation_id, request_id, 
    ...
    async def test_process_terminal_interaction_information(self):
        ...
        # 存储请求信息
        provider._store_request(conversation_id, request_id, task_id, 
                               HumanLoopType.INFORMATION, request_info["context"], 
                               request_info["metadata"], None)
        ...
        with patch('asyncio.get_event_loop') as mock_loop:
            ...
            # 调用处理函数
            await provider._handle_information_interaction(conversation_id, request_id, 
                                                         provider._requests[(conversation_id, request_id)])
    ...
    async def test_process_terminal_interaction_conversation(self):
        ...
        # 存储请求信息
        provider._store_request(conversation_id, request_id, task_id, 
                               HumanLoopType.CONVERSATION, request_info["context"], 
                               request_info["metadata"], None)
        ...
        with patch('asyncio.get_event_loop') as mock_loop:
            ...
            # 调用处理函数
            await provider._handle_conversation_interaction(conversation_id, request_id, 
                                                          provider._requests[(conversation_id, request_id)])
...
class TestHumanLoopRequest(IsolatedAsyncioTestCase):
    ...
    async def test_request_humanloop(self):
        ...
        with patch.object(provider, '_process_terminal_interaction', new_callable=AsyncMock) as mock_process:
            # 发起请求
            ...
            self.assertEqual(request_info["context"], {"message": "请审批此请求", "question": "请审批"})
    ...
    async def test_check_request_status(self):
        ...
        # 更新请求状态
        provider._requests[(conversation_id, request_id)]["status"] = HumanLoopStatus.APPROVED
        provider._requests[(conversation_id, request_id)]["response"] = {"decision": "approved"}
        provider._requests[(conversation_id, request_id)]["responded_by"] = "terminal_user"
        provider._requests[(conversation_id, request_id)]["responded_at"] = datetime.now().isoformat()
    ...
    async def test_continue_humanloop(self):
        ...
        with patch.object(provider, '_process_terminal_interaction', new_callable=AsyncMock) as mock_process:
            ...
            self.assertEqual(request_info["context"], {"message": "感谢您的回复，还有其他问题吗？"})
            self.assertEqual(request_info["metadata"], {"test_key": "test_value"})
            self.assertEqual(request_info["status"], HumanLoopStatus.PENDING)
            self.assertEqual(request_info["timeout"], 60)
...
class TestTerminalInputProcessing(IsolatedAsyncioTestCase):
    ...
    async def test_handle_approval_interaction_invalid_input(self):
        ...
        with patch('asyncio.get_event_loop') as mock_loop:
            ...
            with patch.object(provider, '_handle_approval_interaction', side_effect=mock_handle_approval):
                # 调用处理函数
                await provider._handle_approval_interaction(conversation_id, request_id, 
                                                          provider._requests[(conversation_id, request_id)])
