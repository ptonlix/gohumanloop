...
# 创建 FastAPI 应用
app = FastAPI(
    title="GoHumanLoop Mock Server", description="Mock API for GoHumanLoop service"
)
...
api_router = APIRouter(
    prefix="/api/v1/humanloop", dependencies=[Depends(verify_api_key)]
)
...
async def auto_respond(conversation_id: str, request_id: str, delay: int = 5):
    ...
    if loop_type == "approval":
        ...
        response = {"decision": decision, "reason": f"这是一个自动{decision}的理由"}
        status = "approved" if decision == "approved" else "rejected"
    elif loop_type == "information":
        # 提供一些模拟信息
        response = {"information": "这是一些模拟的信息内容"}
        status = "completed"
    else:  # conversation
        # 检查是否是该对话的第二次请求
        conversation_requests = conversations_store.get(conversation_id, [])
        is_second_request = (
            len(conversation_requests) > 1 and request_id == conversation_requests[-1]
        )

        # 模拟对话响应
        response = {"message": "这是一个自动生成的对话响应"}
...
async def request_humanloop(data: HumanLoopRequestData):
    ...
    task = asyncio.create_task(
        auto_respond(data.conversation_id, data.request_id, delay)
    )
...
async def cancel_request(
    data: HumanLoopCancelData, token: str = Depends(verify_api_key)
):
    """取消人机循环请求"""
    request_key = f"{data.conversation_id}:{data.request_id}"

    if request_key not in requests_store:
        return APIResponse(success=False, error=f"Request not found: {data.request_id}")
...
async def cancel_conversation(
    data: HumanLoopCancelConversationData, token: str = Depends(verify_api_key)
):
    """取消整个对话"""
    if data.conversation_id not in conversations_store:
        return APIResponse(
            success=False, error=f"Conversation not found: {data.conversation_id}"
        )
...
async def continue_humanloop(
    data: HumanLoopContinueData, token: str = Depends(verify_api_key)
):
    """继续人机循环交互"""
    # 检查对话是否存在
    if data.conversation_id not in conversations_store:
...
async def sync_task_data(data: Dict[str, Any], token: str = Depends(verify_api_key)):
    ...
    if not task_id:
        return APIResponse(success=False, error="Missing task_id")
    ...
    if task_id in tasks_store:
        ...
        for new_conv in new_conversations:
            ...
            if conv_id in conv_map:
                ...
                existing_requests = {
                    req.get("request_id"): req
                    for req in existing_conv.get("requests", [])
                }
...
async def get_all_tasks(token: str = Depends(verify_api_key)):
    """获取所有任务数据"""
    return {"success": True, "tasks": tasks_store}
...
async def get_task(task_id: str, token: str = Depends(verify_api_key)):
    ...
    return {"success": True, "task": tasks_store[task_id]}
