...
def generate_response(state: AgentState) -> AgentState:
    ...
    if feedback_history:
        ...
        temp_messages.append(HumanMessage(content=f"请根据以下反馈调整你的回复: {last_feedback}"))
    else:
...
def review_response(state: AgentState, human_feedback: Dict[str, Any] = {}) -> AgentState:
    """审核回复，需要人类反馈"""
    messages = state["messages"]
...
async def run_example():
    ...
    print("实现人机协作的问答流程。\n")
    print("提示: 如果对AI回复满意，请回复'满意'或'通过'结束流程。否则提供具体反馈以改进回复。\n")
    
    # 初始化状态
    state = initialize_state()
    
    # 添加用户问题
    state["messages"].append(HumanMessage(content="请解释量子计算的基本原理和潜在应用。"))
