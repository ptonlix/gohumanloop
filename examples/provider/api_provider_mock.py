...
async def approval_example(provider):
    ...
    while True:
        ...
        status = await provider.async_check_request_status(
            conversation_id=result.conversation_id, request_id=result.request_id
        )
...
async def cancel_request_example(provider):
    ...
    # 取消请求
    cancel_success = await provider.async_cancel_request(
        conversation_id=result.conversation_id, request_id=result.request_id
    )
...
async def cancel_conversation_example(provider):
    ...
    # 发起第一个请求
    result1 = await provider.async_request_humanloop(
    ...
    # 发起第二个请求
    result2 = await provider.async_request_humanloop(
        task_id="DB-PERF-002",
        conversation_id=conversation_id,
        loop_type=HumanLoopType.INFORMATION,
        context=context2,
    ...
    # 取消整个对话
    cancel_success = await provider.async_cancel_conversation(
        conversation_id=conversation_id
    )
    ...
    await asyncio.sleep(3)
    # 检查两个请求的状态
    status1 = await provider.async_check_request_status(
        conversation_id=conversation_id, request_id=result1.request_id
    )

    status2 = await provider.async_check_request_status(
        conversation_id=conversation_id, request_id=result2.request_id
    )
...
async def main():
    ...
    # 运行示例
    example_type = input(
        "请选择要运行的示例 (1: 审批, 2: 获取信息, 3: 多轮对话, 4: 取消请求, 5: 取消对话): "
    )
