...
manager = GoHumanLoopManager(additional_providers=[
...
adapter = LangGraphAdapter(manager, default_timeout=600)

# 使用审批装饰器的敏感操作
@adapter.require_approval(execute_on_reject=True, provider_id="TerminalProvider")
def execute_financial_transaction(amount: float, account_id: str, approval_result=None) -> Dict[str, Any]:
    """执行一个需要人工审批的金融交易
...
@adapter.require_approval(ret_key="approval_data", execute_on_reject=True, provider_id="EmailProvider" ,metadata={"recipient_email": recipient_email})
...
def collect_feedback(state: AgentState, feedback_data={}) -> AgentState:
    ...
    feedback_message = HumanMessage(content=f"收到用户反馈: {feedback_data.get('response', '无反馈')}")
...
def main():
    ...
    try:
        # 创建一个更符合金融投资场景的初始状态
        initial_state = {
            "messages": [HumanMessage(content="我想投资5万元购买一些低风险的基金产品，主要用于退休储蓄，期限大约10年。请给我一些建议？")],
            "next": "agent"
        }
